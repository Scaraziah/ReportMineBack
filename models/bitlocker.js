...
function validateBitlocker(bitlocker) {
    const schema =Joi.object({
        computerName: Joi.string().min(1).max(150).required(),
        serviceTag: Joi.string().min(1).max(255).required(),
        currentUser: Joi.string().min(1).max(255),
        remoteOffice: Joi.string().min(1).max(1024).required(),
        driveName: Joi.string().min(1).max(1024).required(),
        driveType: Joi.string().min(1).max(1024).required(),
        proStatus: Joi.string().min(1).max(1024).required(),
        encryStatus: Joi.string().min(1).max(1024).required(),
        lockStatus: Joi.string().min(1).max(1024).required(),
        encryMethod: Joi.string().min(1).max(1024).required(),
        recKey: Joi.string().min(1).max(1024).required(),
    });
